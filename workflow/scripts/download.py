...
if source == partial:
    if kind == "microsoft":
        links = pd.read_csv(snakemake.input.index, dtype={"QuadKey": str})
        urls = sorted(links.loc[links.QuadKey.eq(snakemake.wildcards.quadkey), "Url"])
        url = urls[int(snakemake.wildcards.part)]
    else:
...
if kind == "nuts3":
    ...
    validate_census(source, snakemake.params.year)
elif kind == "building_age":
    validate_building_age_census(source, snakemake.params.year)
elif kind == "population":
    # Inspect the GeoTIFF in place; extraction remains a separate reusable job.
    validate_population_raster(
        f"/vsizip/{source.resolve()}/{snakemake.params.member}",
    ...
    validate_eubucco_source(source, snakemake.params.source)
else:
